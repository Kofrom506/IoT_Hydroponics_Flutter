...
class DateParser {
  ...
  static String parseTimeChat(DateTime dateTime) {
    var localDate = dateTime.toLocal();
    var formatter = NumberFormat("00");
    return "${formatter.format(localDate.day)}-${formatter.format(localDate.month)}-${formatter.format(localDate.year)} ${formatter.format(localDate.hour)}:${formatter.format(localDate.minute)}";
  }
